...
const ProductTimeSale = () => {
    ...
    try {
      ...
      const transformedTimeSales = response.map(item => {
        return {
          ...
          endDateTime: moment(item.endDateTime).tz("Asia/Seoul").format("YYYY-MM-DD HH:mm"),
          price: item.price,
          productId: item.productId,
          productName: item.productName,
          serviceStatus: item.serviceStatus,
          startDateTime: moment(item.startDateTime).tz("Asia/Seoul").format("YYYY-MM-DD HH:mm"),
          timesaleId: item.timesaleId,
        };
      });

      console.log(transformedTimeSales);
  ...
  const columns = [
    ...
    },
    {
      ...
      render: status => <Tag color={getTagColor(status)}>{getTagText(status)}</Tag>,
    },
  ];
