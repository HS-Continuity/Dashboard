...
const ProductGeneral = () => {
  ...
  const onHandleTimesaleApply = async () => {
    ...
    const { value: formValues } = await Swal.fire({
      ...
      html:
        ...
        "</div>" +
        '<div style="text-align: left; margin-bottom: 5px;">' +
        '<label for="swal-input2" style="display: inline-block; width: 150px; font-weight: bold;">타임세일 시작 (KST)</label>' +
        ...
        '<div style="text-align: left; margin-bottom: 20px;">' +
        '<label for="swal-input3" style="display: inline-block; width: 150px; font-weight: bold;">타임세일 종료 (KST)</label>' +
    ...
    if (formValues) {
      ...
      try {
        ...
        console.log("Sending to server:", timesaleData);
  ...
  return (
    <div>
      ...
      <Flex gap='small' align='center' justify='space-between'>
        <Flex gap='small' wrap>
          <Button onClick={onHandleReset} className={styles.smallButton}>
            초기화
          </Button>
          {/* <Button onClick={clearAll}>Clear filters and sorters</Button> */}
        </Flex>
        <Flex gap='small' wrap>
          <RegisterButton title={"일반식품"} onClick={onClickCreate} />
          <ApplyButton title={"타임세일"} onClick={onHandleTimesaleApply} />
          <PromotionApplyButton title={"상단 노출"} onClick={onHandlePromotionApply} />
        </Flex>
      </Flex>
      <br />
      <ConfigProvider
